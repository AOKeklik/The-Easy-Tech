                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form data-app-form="caseStudy-store">
                            <input type="hidden" id="caseStudy_id" name="caseStudy_id" value="{{ request("caseStudy_id") }}">
                            <div class="mb-3">
                                <label for="category_id" class="form-label">Category*</label>
                                <select class="form-select form-select-lg"  name="category_id" id="category_id">
                                    <option value="">Select One</option>
                                    @foreach($categories as $cat)
                                        <option @if($caseStudy->category_id === $cat->id) selected @endif value="{{ $cat->id }}">{{ $cat->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="slug" name="slug" readonly value="{{ $caseStudy->slug }}">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea id="desc" name="desc" class="summernote form-control" rows="5" spellcheck="false">{{ $caseStudy->desc }}</textarea>
                                <small data-app-alert="caseStudy-store-desc" class="form-text text-danger"></small>
                            </div> 
                            <div class="mb-3">
                                <label for="seo_title" class="form-label">Seo Title</label>
                                <input type="text" class="form-control" id="seo_title" name="seo_title" value="{{ $caseStudy->seo_title }}">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea id="seo_desc" name="seo_desc" class="summernote form-control" rows="5" spellcheck="false">{{ $caseStudy->seo_desc }}</textarea>
        ...
        $(document).ready(function(){
            ...
            async function handlerSubmit (e) {
                try {
                    ...
                    formData.append("caseStudy_id",form.find("#caseStudy_id").val())
            ...
            async function submit(formData){
                try {
                    ...
                    const result = await $.ajax({
                        type: "POST",
                        url: "{{ route('admin.caseStudy.update') }}",
                        data: formData,
                        contentType: false,
                        processData: false,
                    });
