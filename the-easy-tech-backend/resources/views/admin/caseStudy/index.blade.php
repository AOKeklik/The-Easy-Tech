            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a href="{{ route("admin.caseStudy.add.view") }}" class="btn btn-primary">Add</a>
                    </div><!-- end card header -->
                    <div class="card-body" data-app-section="caseStudy-table">
                        @include("admin.caseStudy.table")
                    </div>
                </div>
            </div>
        </div>
...
@push("scripts")
    <script>
        $(document).ready(function(){
            $(document)
                .on("change","[data-app-btn=caseStudy-status-update]",handlerStatusUpdate)
                .on("click","[data-app-btn=caseStudy-delete]",handlerDelete)
            ...
            async function handlerStatusUpdate (e) {
                try {
                    ...
                    formData.append("caseStudy_id",$(this).data("casestudy-id"))
            ...
            async function handlerDelete (e) {
                try {
                    ...
                    const update=await deleteItem(formData)
                    await fetchData()
            ...
            async function updateStatus(formData){
                try {
                    ...
                    const result = await $.ajax({
                        type: "POST",
                        url: "{{ route('admin.caseStudy.status.update') }}",
                        data: formData,
                        contentType: false,
                        processData: false,
                    });
            ...
            async function deleteItem(formData){
                try {
                    ...
                    const result = await $.ajax({
                        type: "POST",
                        url: "{{ route('admin.caseStudy.delete') }}",
                        data: formData,
                        contentType: false,
                        processData: false,
                    });
            ...
            function fetchData(){
                $.ajax({
                    type:"GET",
                    url:"{{ route('admin.caseStudy.table.view') }}",
                    success:function(res){
                        $("[data-app-section=caseStudy-table]").html(res)
                        reloadJqueryPlugins ()
                    },
                    error: function(xhr) {
